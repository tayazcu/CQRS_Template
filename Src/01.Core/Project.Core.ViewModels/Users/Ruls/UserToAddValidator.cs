...
{
    public class UserToAddValidator : AbstractValidator<UserToAddVM>, IScopedDependency
    {
        private readonly IResourceManager _resourceManager;
        public UserToAddValidator(IResourceManager resourceManager)
        {
            ...
            RuleFor(x => x.UserName)
               ...
               .Length(2, 250)
                   .WithMessage($"{_resourceManager.GetName(SharedResource.UserName)} {_resourceManager.GetName(SharedResource.Between7And400) }");

            RuleFor(x => x.Password)
               .NotNull()
                   .WithMessage($"{_resourceManager.GetName(SharedResource.Password)} {_resourceManager.GetName(SharedResource.Required) }")
               .Length(2, 250)
                   .WithMessage($"{_resourceManager.GetName(SharedResource.Password)} {_resourceManager.GetName(SharedResource.Between7And400) }");
            ...
            RuleFor(x => x.UserType.ToString())
              .NotNull()
                  .WithMessage($"{_resourceManager.GetName(SharedResource.UserType)} {_resourceManager.GetName(SharedResource.Required) }")
              .Length(2, 50)
                  .WithMessage($"{_resourceManager.GetName(SharedResource.UserType)} {_resourceManager.GetName(SharedResource.Between2And50) }");
